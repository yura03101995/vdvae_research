...
def training_step(H, data_input, target, vae, ema_vae, optimizer, iterate, lambda_mult=None, optimizer_lambda=None):
    t0 = time.time()
    vae.zero_grad()
...
def train_loop(H, data_train, data_valid, preprocess_fn, vae, ema_vae, logprint):
    ...
    for epoch in range(starting_epoch, H.num_epochs):
        ...
        for x in DataLoader(data_train, batch_size=H.n_batch, drop_last=True, pin_memory=True, sampler=train_sampler):
            ...
            training_stats = training_step(H, data_input, target, vae, ema_vae, optimizer, iterate, lambda_mult, optimizer_lambda)
...
def run_draw_uncond_samples(H, ema_vae, logprint):
    ...
    logscales = logscales.detach().cpu().numpy().ravel()
    ...
    imageio.imwrite(f'{H.save_dir}/{H.exp_name}_samples.png', im)
