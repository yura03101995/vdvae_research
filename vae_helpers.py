...
def discretized_mix_logistic_loss(x, l, low_bit=False):
    ...
    l = torch.reshape(l[:, :, :, nr_mix:], xs + [nr_mix * 3]) # xs + [nr_mix * 3] = (B, 32, 32, 3, num_mixtures * 3)
    means = l[:, :, :, :, :nr_mix]
    ...
    if low_bit:
        ...
        minus_in = inv_stdv * (centered_x - 1. / 31.)
    else:
        plus_in = inv_stdv * (centered_x + 1. / 255.)
        minus_in = inv_stdv * (centered_x - 1. / 255.)

    cdf_plus = torch.sigmoid(plus_in)
    cdf_min = torch.sigmoid(minus_in)
    log_cdf_plus = plus_in - F.softplus(plus_in)  # log probability for edge case of 0 (before scaling)
    log_one_minus_cdf_min = -F.softplus(minus_in)  # log probability for edge case of 255 (before scaling)
    cdf_delta = cdf_plus - cdf_min  # probability for all other cases
    middle_in = inv_stdv * centered_x
    log_pdf_mid = middle_in - log_scales - 2. * F.softplus(middle_in)  # log probability in the center of the bin, to be used in extreme cases (not actually used in our code)
...
def sample_mean_from_discretized_mix_logistic(l, nr_mix):
    ls = [s for s in l.shape]
    xs = ls[:-1] + [3]
    # unpack parameters
    probs = torch.exp(log_prob_from_logits(l[:, :, :, :nr_mix]))
    probs = torch.cat(3 * [probs.unsqueeze(dim=3)], dim=3)
    l = torch.reshape(l[:, :, :, nr_mix:], xs + [nr_mix * 3])
    means = l[:, :, :, :, :nr_mix]
    log_scales = const_max(l[:, :, :, :, nr_mix:nr_mix * 2], -7.)
    coeffs = torch.tanh(l[:, :, :, :, 2 * nr_mix:3 * nr_mix])

    # calculate means, coeffs
    means = (means * probs).sum(dim=-1)
    coeffs = (coeffs * probs).sum(dim=-1)
    ...
    x1 = const_min(const_max(x[:, :, :, 1] + coeffs[:, :, :, 0] * x0, -1.), 1.)
    x2 = const_min(const_max(x[:, :, :, 2] + coeffs[:, :, :, 1] * x0 + coeffs[:, :, :, 2] * x1, -1.), 1.)
    return torch.cat([torch.reshape(x0, xs[:-1] + [1]), torch.reshape(x1, xs[:-1] + [1]), torch.reshape(x2, xs[:-1] + [1])], dim=3), log_scales, probs
...
def sample_distortion_loss(l):
    return const_min(const_max(l, -1.), 1.)
...
class DmolNet(nn.Module):
    ...
    def sample(self, px_z, return_logscales_probs=False):
        # im = sample_from_discretized_mix_logistic(self.forward(px_z), self.H.num_mixtures)
        im, log_scales, probs = sample_mean_from_discretized_mix_logistic(self.forward(px_z), self.H.num_mixtures)
        xhat = (im + 1.0) * 127.5
...
class DistortionNet(nn.Module):
    ...
    def sample(self, px_z, return_logscales_probs=False):
        im = sample_distortion_loss(self.forward(px_z))
        xhat = (im + 1.0) * 127.5
        xhat = xhat.detach().cpu().numpy()
        xhat = np.minimum(np.maximum(0.0, xhat), 255.0).astype(np.uint8)
        return xhat
