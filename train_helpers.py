...
class ConstantMult(nn.Module):
    ...
    def __init__(self):
        ...
        self.lambda_rate = nn.Parameter(torch.tensor([0.9999]))

    def forward(self, X):
        """Implement function to be optimised. In this case, an exponential decay
        function (lambda_rate * X),
        """
        return self.lambda_rate * X
...
def setup_save_dirs(H):
    H.save_dir = os.path.join(H.save_dir, H.desc, H.exp_name)
    mkdir_p(H.save_dir)
    mkdir_p(os.path.join(H.save_dir, 'samples'))
